...
("use strict");
{
    ...
    window.RuntimeInterface = class RuntimeInterface {
        ...
        async _InitWorker(opts, port2) {
            ...
            } else this._worker = await this.CreateWorker(workerMainUrl, this._runtimeBaseUrl, { type: "module", name: "Runtime", isC3MainWorker: true });
...
("use strict");
{
    ...
    const HANDLER_CLASS = class RuntimeDOMHandler extends self.DOMHandler {
        ...
        _OnWindowResize() {
            ...
            this.PostToRuntime("window-resize", { innerWidth: width, innerHeight: height, devicePixelRatio: window.devicePixelRatio, isFullscreen: RuntimeInterface.IsDocumentFullscreen() });
        ...
        _OnExportedToVideo(e) {
            window.c3_postToMessagePort({ type: "exported-video", blob: e["blob"], time: e["time"] });
        }
    ...
    self.AudioDOMHandler = class AudioDOMHandler extends self.DOMHandler {
        ...
        async _CreateAudioContext(e) {
            ...
            if (this._iRuntime.IsExportingToVideo()) {
                ...
                this._audioContext = new OfflineAudioContext({ numberOfChannels: 2, sampleRate: sampleRate, length: Math.ceil(this._iRuntime.GetExportToVideoDuration() * sampleRate) });
            } else {
        ...
        *audioInstancesByEffectTag(tag) {
        ...
        Tick() {
            ...
            this.PostToRuntime("state", { tickCount: this._lastTickCount, outputLatency: this._audioContext["outputLatency"] || 0, audioInstances: instStates, analysers: [...this._analysers].map((a) => a.GetData()) });
        ...
        _OnOfflineRenderCompleted(buffer) {
            ...
            this._iRuntime.PostToRuntimeComponent(
                ...
                { duration: buffer["duration"], length: buffer["length"], numberOfChannels: buffer["numberOfChannels"], sampleRate: buffer["sampleRate"], channelData: channelArrayBuffers },
                null,
                channelArrayBuffers
            );
        }
